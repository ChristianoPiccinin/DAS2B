...
class ExtractRepresentanteTrigger(BaseTrigger):
    """Trigger for extracting representante data from ERP to Azure SQL."""

    def __init__(self):
        super().__init__(trigger_name='extract_representante')

    def run(self, myTimer: func.TimerRequest) -> None:
        """Execute the representante extraction pipeline."""
        ...
        try:
            ...
            with pyodbc.connect(conn_str_source) as conn_source:
                ...
                cursor_source.execute("SELECT * FROM erp.representante")
                rows = cursor_source.fetchall()
                columns = [description[0] for description in cursor_source.description]

            if not rows:
                logging.warning('Nenhum registro encontrado na tabela representante')
                return
            ...
            with pyodbc.connect(conn_str_dest) as conn_dest:
                ...
                insert_query = f"INSERT INTO erp.representante ({','.join(columns)}) VALUES ({placeholders})"
...
def extract_representante(myTimer: func.TimerRequest) -> None:
    """Azure Function entry point for representante extraction."""
    trigger = ExtractRepresentanteTrigger()
    trigger.run(myTimer)
