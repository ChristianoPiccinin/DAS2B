...
class ExtractTransportadoraTrigger(BaseTrigger):
    """Trigger for extracting transportadora data from ERP to Azure SQL."""

    def __init__(self):
        super().__init__(trigger_name='extract_transportadora')

    def run(self, myTimer: func.TimerRequest) -> None:
        """Execute the transportadora extraction pipeline."""
        ...
        try:
            ...
            with pyodbc.connect(conn_str_source) as conn_source:
                ...
                cursor_source.execute("SELECT * FROM erp.transportadora")
                rows = cursor_source.fetchall()
                columns = [description[0] for description in cursor_source.description]

            if not rows:
                logging.warning('Nenhum registro encontrado na tabela transportadora')
                return
            ...
            with pyodbc.connect(conn_str_dest) as conn_dest:
                ...
                cursor_dest.execute("DELETE FROM erp.transportadora")
                logging.info('Tabela de destino limpa')

                placeholders = ','.join(['?' for _ in columns])
                insert_query = f"INSERT INTO erp.transportadora ({','.join(columns)}) VALUES ({placeholders})"
...
def extract_transportadora(myTimer: func.TimerRequest) -> None:
    """Azure Function entry point for transportadora extraction."""
    trigger = ExtractTransportadoraTrigger()
    trigger.run(myTimer)
