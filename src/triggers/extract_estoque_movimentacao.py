...
class ExtractEstoqueMovimentacaoTrigger(BaseTrigger):
    """Trigger for extracting estoque_movimentacao data from ERP to Azure SQL."""

    def __init__(self):
        super().__init__(trigger_name='extract_estoque_movimentacao')

    def run(self, myTimer: func.TimerRequest) -> None:
        """Execute the estoque_movimentacao extraction pipeline."""
        ...
        try:
            ...
            with pyodbc.connect(conn_str_source) as conn_source:
                ...
                cursor_source.execute("SELECT * FROM erp.estoque_movimentacao")
                rows = cursor_source.fetchall()
                columns = [description[0] for description in cursor_source.description]

            if not rows:
                logging.warning('Nenhum registro encontrado na tabela estoque_movimentacao')
                return
            ...
            with pyodbc.connect(conn_str_dest) as conn_dest:
                ...
                insert_query = f"INSERT INTO erp.estoque_movimentacao ({','.join(columns)}) VALUES ({placeholders})"
...
def extract_estoque_movimentacao(myTimer: func.TimerRequest) -> None:
    """Azure Function entry point for estoque_movimentacao extraction."""
    trigger = ExtractEstoqueMovimentacaoTrigger()
    trigger.run(myTimer)
