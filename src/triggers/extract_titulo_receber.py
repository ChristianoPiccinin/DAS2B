...
class ExtractTituloReceberTrigger(BaseTrigger):
    """Trigger for extracting titulo_receber data from ERP to Azure SQL."""

    def __init__(self):
        super().__init__(trigger_name='extract_titulo_receber')

    def run(self, myTimer: func.TimerRequest) -> None:
        """Execute the titulo_receber extraction pipeline."""
        ...
        try:
            ...
            with pyodbc.connect(conn_str_source) as conn_source:
                ...
                cursor_source.execute("SELECT * FROM erp.titulo_receber")
                rows = cursor_source.fetchall()
                columns = [description[0] for description in cursor_source.description]

            if not rows:
                logging.warning('Nenhum registro encontrado na tabela titulo_receber')
                return
            ...
            with pyodbc.connect(conn_str_dest) as conn_dest:
                ...
                cursor_dest.execute("DELETE FROM erp.titulo_receber")
                logging.info('Tabela de destino limpa')

                placeholders = ','.join(['?' for _ in columns])
                insert_query = f"INSERT INTO erp.titulo_receber ({','.join(columns)}) VALUES ({placeholders})"
...
def extract_titulo_receber(myTimer: func.TimerRequest) -> None:
    """Azure Function entry point for titulo_receber extraction."""
    trigger = ExtractTituloReceberTrigger()
    trigger.run(myTimer)
