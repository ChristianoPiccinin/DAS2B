...
class ExtractEstoquesSaldoTrigger(BaseTrigger):
    """Trigger for extracting estoque_saldo data from ERP to Azure SQL."""

    def __init__(self):
        super().__init__(trigger_name='extract_estoque_saldo')

    def run(self, myTimer: func.TimerRequest) -> None:
        """Execute the estoque_saldo extraction pipeline."""
        ...
        try:
            ...
            with pyodbc.connect(conn_str_source) as conn_source:
                ...
                cursor_source.execute("SELECT * FROM erp.estoque_saldo")
                rows = cursor_source.fetchall()
                columns = [description[0] for description in cursor_source.description]

            if not rows:
                logging.warning('Nenhum registro encontrado na tabela estoque_saldo')
                return
            ...
            with pyodbc.connect(conn_str_dest) as conn_dest:
                ...
                cursor_dest.execute("DELETE FROM erp.estoque_saldo")
                logging.info('Tabela de destino limpa')

                placeholders = ','.join(['?' for _ in columns])
                insert_query = f"INSERT INTO erp.estoque_saldo ({','.join(columns)}) VALUES ({placeholders})"
...
def extract_estoque_saldo(myTimer: func.TimerRequest) -> None:
    """Azure Function entry point for estoque_saldo extraction."""
    trigger = ExtractEstoquesSaldoTrigger()
    trigger.run(myTimer)
