...
class ExtractEntregaTrigger(BaseTrigger):
    """Trigger for extracting entrega data from ERP to Azure SQL."""

    def __init__(self):
        super().__init__(trigger_name='extract_entrega')

    def run(self, myTimer: func.TimerRequest) -> None:
        """Execute the entrega extraction pipeline."""
        ...
        try:
            ...
            with pyodbc.connect(conn_str_source) as conn_source:
                ...
                cursor_source.execute("SELECT * FROM erp.entrega")
                rows = cursor_source.fetchall()
                columns = [description[0] for description in cursor_source.description]

            if not rows:
                logging.warning('Nenhum registro encontrado na tabela entrega')
                return
            ...
            with pyodbc.connect(conn_str_dest) as conn_dest:
                ...
                cursor_dest.execute("DELETE FROM erp.entrega")
                logging.info('Tabela de destino limpa')

                placeholders = ','.join(['?' for _ in columns])
                insert_query = f"INSERT INTO erp.entrega ({','.join(columns)}) VALUES ({placeholders})"
...
def extract_entrega(myTimer: func.TimerRequest) -> None:
    """Azure Function entry point for entrega extraction."""
    trigger = ExtractEntregaTrigger()
    trigger.run(myTimer)
