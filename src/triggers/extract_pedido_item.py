...
def extract_pedido_item(myTimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        with pyodbc.connect(conn_str_source) as conn_source:
            ...
            cursor_source.execute("SELECT * FROM erp.pedido_item")
            rows = cursor_source.fetchall()
            columns = [description[0] for description in cursor_source.description]

        if not rows:
            logging.warning('Nenhum registro encontrado na tabela pedido_item')
            return
        ...
        with pyodbc.connect(conn_str_dest) as conn_dest:
            ...
            cursor_dest.execute("DELETE FROM itsm.pedido_item")
            logging.info('Tabela de destino limpa')

            placeholders = ','.join(['?' for _ in columns])
            insert_query = f"INSERT INTO erp.pedido_item ({','.join(columns)}) VALUES ({placeholders})"
