...
class ExtractFornecedorTrigger(BaseTrigger):
    """Trigger for extracting fornecedor data from ERP to Azure SQL."""

    def __init__(self):
        super().__init__(trigger_name='extract_fornecedor')

    def run(self, myTimer: func.TimerRequest) -> None:
        """Execute the fornecedor extraction pipeline."""
        ...
        try:
            ...
            logging.info('tabela fornecedor.')
            ...
            with pyodbc.connect(conn_str_source) as conn_source:
                ...
                cursor_source.execute("SELECT * FROM erp.fornecedor")
                rows = cursor_source.fetchall()
                columns = [description[0] for description in cursor_source.description]

            if not rows:
                logging.warning('Nenhum registro encontrado na tabela fornecedor')
                return
            ...
            with pyodbc.connect(conn_str_dest) as conn_dest:
                ...
                cursor_dest.execute("DELETE FROM erp.fornecedor")
                logging.info('Tabela de destino limpa')

                placeholders = ','.join(['?' for _ in columns])
                insert_query = f"INSERT INTO erp.fornecedor ({','.join(columns)}) VALUES ({placeholders})"
...
def extract_fornecedor(myTimer: func.TimerRequest) -> None:
    """Azure Function entry point for fornecedor extraction."""
    trigger = ExtractFornecedorTrigger()
    trigger.run(myTimer)
