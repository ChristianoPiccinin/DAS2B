...
def extract_regiao(myTimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        with pyodbc.connect(conn_str_source) as conn_source:
            ...
            cursor_source.execute("SELECT * FROM erp.regiao")
            rows = cursor_source.fetchall()
            columns = [description[0] for description in cursor_source.description]

        if not rows:
            logging.warning('Nenhum registro encontrado na tabela regiao')
            return
        ...
        with pyodbc.connect(conn_str_dest) as conn_dest:
            ...
            cursor_dest.execute("DELETE FROM itsm.regiao")
            logging.info('Tabela de destino limpa')

            placeholders = ','.join(['?' for _ in columns])
            insert_query = f"INSERT INTO erp.regiao ({','.join(columns)}) VALUES ({placeholders})"
